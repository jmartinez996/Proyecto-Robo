...
export default function Tablaareas() {
	...
	const getAreas = async () => {
		const areas = await axios(`http://10.13.18.84:5005/getAreas`, {
			headers: {
	...
	const showAlert = () => {
		...
		MySwal.fire({
			...
			preConfirm: (nombre) => {
				...
				return axios
					.post(`http://10.13.18.84:5005/createArea/`, f, { headers: { "Content-Type": "application/json", Authorization: `Bearer ` + token } })
					.then((response) => {
	...
	const delete_Area = ($id, $name) => {
		...
		MySwal.fire({
			...
			.then((result) => {
				if (result.isConfirmed) {
					axios
						.post(`http://10.13.18.84:5005/deleteArea/`, f, {
							headers: {
								"Content-Type": "application/json",
								Authorization: `Bearer ` + token,
							},
					...
					MySwal.fire("Eliminado", "El registro " + name + " se ha eliminado", "success");
				} else if (
		...
		/*
      return axios.post(`http://10.13.18.84:5005/deleteArea/`, f, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer `+token
        }}).then(response=>{
								...
								<TableRow hover key={area && area.id_area}>
									...
									{context.role === "sudo" && (
										<TableCell>
											...
											<IconButton aria-label='Editar' onClick={() => change_Area(area.id_area, area.nombre_area)}>
												<EditIcon />
											</IconButton>
										</TableCell>
									)}
