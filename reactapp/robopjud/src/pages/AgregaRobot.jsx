...
function AgregaRobot() {
	...
	useEffect(async () => {
		await axios
			.get(`http://127.0.0.1:5000/getAreas`, {
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ` + token,
				},
							...
							render={({ field: { onChange, value }, fieldState: { error } }) => (
								<>
									<FormControl variant='outlined' className={classes.formControl} fullWidth>
										...
										<Select labelId='demo-simple-select-outlined-label' id='demo-simple-select-outlined' label='Age' margin='dense' error={!!error} helperText={error ? error.message : null} onChange={onChange} name='area'>
											{areas.map((area) => (
